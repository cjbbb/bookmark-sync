...
export class AIEngine {
  ...
  /**
   * Classify a single batch of bookmarks using the configured AI provider.
   * @param {Array} batch - Subset of bookmarks to classify
   * @param {Object} context - { names: string[], _allPaths: string[] }
   * @returns {Promise<Array>}
   */
  static async classifyBatch(batch, context) {
    const provider = await this.getProvider();
    return provider._classifyBatch(batch, context);
  }
