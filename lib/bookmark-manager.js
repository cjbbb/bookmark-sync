...
export class BookmarkManager {
  ...
  static async importFromSyncFormat(syncData, options = {}) {
    ...
    const bookmarks = this._getValidSyncBookmarks(syncData);

    if (mode === 'overwrite') {
      return this._restoreFromSyncFormat(syncData, bookmarks);
    }
   ...
   * @returns {Promise<{added: number, skipped: number, moved: number, updated: number, removed: number}>}
   */
  static async _restoreFromSyncFormat(syncData, snapshot) {
    const existing = await this.getAllBookmarks();
  ...
  /**
   * Validate sync payload before import to avoid destructive restores
   * on malformed remote data.
   * @param {Object} syncData
   * @returns {Array}
   */
  static _getValidSyncBookmarks(syncData) {
    if (!syncData || typeof syncData !== 'object') {
      throw new Error('同步数据格式无效，已中止导入以保护本地书签。');
    }

    if (!Array.isArray(syncData.bookmarks)) {
      throw new Error('同步数据缺少 bookmarks 列表，已中止导入以保护本地书签。');
    }

    const bookmarks = syncData.bookmarks.filter(
      (bookmark) => bookmark && typeof bookmark.url === 'string' && bookmark.url.trim()
    );

    if (syncData.bookmarks.length > 0 && bookmarks.length === 0) {
      throw new Error('同步数据中的书签条目无效，已中止导入以保护本地书签。');
    }
